  ...
  <header class="bg-gray-900 sm:flex sm:items-center sm:justify-between xl:bg-white">
      ...
      <nav class="sm:flex sm:items-center sm:px-4 xl:flex-1 xl:justify-between" :class="{ 'hidden': !isOpen, 'block': isOpen}">
        <div class="hidden xl:block xl:relative xl:max-w-xs xl:w-full">
          ...
          <input placeholder="Search by keywords" class="block w-full bg-gray-200 text-gray-900 border border-transparent rounded-lg px-10 pr-4 py-2 focus:outline-none focus:bg-white focus:text-gray-600 focus:border-gray-300">
        </div>
        <div class="sm:flex sm:items-center">
          <div class="px-2 pt-2 pb-5 border-gray-800 border-b sm:flex sm:border-b-0 sm:py-0 sm:px-0">
            <a href="#" class="fblock px-3 py-1 rounded font-semibold text-white hover:bg-gray-800 sm:text-sm sm:px-2 xl:text-gray-900 xl:hover:bg-gray-200">List your property</a>
            <a href="#/Trips" class="mt-1 block px-3 py-1 rounded font-semibold text-white hover:bg-gray-800 sm:mt-0 sm:text-sm sm:px-2 sm:ml-2 xl:text-gray-900 xl:hover:bg-gray-200">Trips</a>
          ...
          <div class="relative px-5 py-5 sm:py-0 sm:ml-4 sm:px-0">
            <div class="flex items-center sm:hidden">
              <img class="h-10 w-10 object-cover rounded-full border-2 border-gray-600" src="https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=250&q=80" alt="">
              <span class="text-gray-200 font-semibold ml-4 sm:hidden">Isla Schoger</span>
            </div>
            <div class="mt-5 sm:hidden">
              <a href="#" class="text-gray-400 leading-tight block hover:text-white">Account Settings</a>
              <a href="#" class="text-gray-400 leading-tight block hover:text-white mt-3">Support</a>
              <a href="#" class="text-gray-400 leading-tight block hover:text-white mt-3">Sign Out</a>
            </div>
            <Dropdown class="hidden sm:block">
              <template #trigger="{hasFocus, isOpen}">
                <span class="block rounded-full border-2 overflow-hidden h-8 w-8"
                  :class="[ (hasFocus || isOpen) ? 'border-white xl:border-indigo-500' : 'border-gray-600 xl:border-gray-300']"
                >
                  <img class="object-cover h-full w-full" src="https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=250&q=80" alt="">
                </span>
              </template>
              <template #dropdown>
                <div class="mt-3 bg-white rounded-lg w-48 shadow-xl py-2 xl:border">
                  <a href="#" class="leading-tight block hover:text-white text-gray-800 hover:bg-indigo-500 px-4 py-2 mt-0">Account Settings</a>
                  <a href="#" class="leading-tight block hover:text-white text-gray-800 hover:bg-indigo-500 px-4 py-2 mt-0">Support</a>
                  <a href="#" class="leading-tight block hover:text-white text-gray-800 hover:bg-indigo-500 px-4 py-2 mt-0">Sign Out</a>
                </div>
              </template>
            </Dropdown>
          </div>
